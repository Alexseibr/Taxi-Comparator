...
import { tryMatchCalib } from "./recommend-matcher.mjs";
// MATCH_REC_PATCH_v1
  ...
  "tariffs": [
    {
      ...
      "etaMin": число минут ПОДАЧИ (через сколько водитель приедет к точке А) или null. ВИЗУАЛЬНЫЕ МАРКЕРЫ etaMin в Yandex Go: (а) жёлтая овальная плашка на КАРТЕ возле точки А с иконкой автомобиля и текстом вида «5 мин» — это НА КАРТЕ, не в списке тарифов; рядом часто видна подпись «прибытие в ЧЧ:ММ»; (б) маленькая цифра в КАЖДОЙ карточке тарифа в нижней панели прокрутки тарифов (плашки «Эконом / Комфорт / Электро / Комфорт+»), вверху карточки указано например «5 мин» — это СВОЯ подача для этого тарифа; в JSON используй etaMin тарифа Эконом, либо первое значение тарифа который выбран. etaMin обычно МАЛЕНЬКИЙ (1-15 мин). НЕ ПУТАЙ etaMin с tripMin: если видишь «прибытие в ЧЧ:ММ» — это именно etaMin (подача), а не время поездки.,
      "tripMin": число минут В ПУТИ от точки А до точки Б (длительность САМОЙ поездки с пробками) или null. ВИЗУАЛЬНЫЕ МАРКЕРЫ tripMin в Yandex Go: (а) подпись справа от строки промежуточного адреса/маршрута в средней панели карты, формата «Главная Минская велодорожка · 6 мин» — цифра ПОСЛЕ точки-разделителя «·» это tripMin; (б) либо подпись возле адреса «куда» вида «· 22 мин»; (в) либо в верхней части над картой «23 мин · 8 км». tripMin это длительность маршрута А→Б, обычно 5-90 мин для Минска. ВАЖНО: если на скрине видны ОБА числа (например в жёлтой плашке «5 мин» И в строке маршрута «· 6 мин») — первое это etaMin, второе это tripMin; никогда не клади число из жёлтой плашки с автомобилем в tripMin. Если значение «N мин» только ОДНО на экране и оно стоит в плашке тарифа или жёлтой плашке возле машинки — это etaMin (а tripMin = null), а НЕ tripMin. Если ОДНО «N мин» стоит в строке маршрута — это tripMin (а etaMin тогда возьми из карточки тарифа Эконом).,
    ...
    JSON.stringify(
      {
        ...
        tripMin: calibRecord.tripMin ?? null,
...
async function processOne(filename) {
  ...
  const calibRecord = {
    ...
    // Имя оператора, которое сотрудник ввёл в FAB при загрузке этой серии
    // (или подставленное автоматически по IP, если уже знакомили его раньше).
    // null, если поле не пришло (старые скрины до релиза T012).
    operator: typeof meta.operator === "string" && meta.operator ? meta.operator : null,
    notes:
      `Распознано из ${meta.originalName || filename} моделью ${v.model}. ` +
      `App=${p.appName || "?"}. orderAt=${orderAtSource}` +
      `${screenLocalTime ? ` (статусбар ${screenLocalTime})` : ""}` +
      `${meta.operator ? `. Оператор: ${meta.operator}` : ""}.`,
  };
  ...
  // Связка с рекомендацией (если таксист только что тапнул пару А→Б
  // в нашем приложении и затем прислал скрин). Берём по IP клиента в
  // окне ≤3 мин до загрузки + проверяем близость координат к ожидаемым.
  // Если матч найден — у нас в попапе соты/админке появляется значок 🎯.
  try {
    const uploadedAtMs = calibRecord.uploadedAt
      ? Date.parse(calibRecord.uploadedAt) : Date.now();
    const matched = await tryMatchCalib({
      uploadedAtMs,
      receivedFromIp: calibRecord.receivedFromIp,
      fromLat: calibRecord.fromLat,
      fromLng: calibRecord.fromLng,
      toLat: calibRecord.toLat,
      toLng: calibRecord.toLng,
    });
    if (matched) {
      calibRecord.matchedRecommendation = matched;
      console.log(`  🎯 matched recommendation: ${matched.expectedFromName} → ${matched.expectedToName} (${matched.confidence}, +${(matched.deltaMs/1000).toFixed(0)}s, fromΔ=${matched.fromDistanceM}м, toΔ=${matched.toDistanceM}м)`);
    }
  } catch (e) {
    console.warn(`  ! match-rec failed: ${e?.message || e}`);
  }
